...
public class CharacterUpgradeService : ICharacterUpgradeService
{
    ...
    public async Task<bool> UpsertUpgradeAsync(UpgradeRequest update, CancellationToken token = default)
    {
        ...
        switch (update.UpgradeOption)
        {
            ...
            case UpgradeOption.attribute4:
                ...
                {
                    ImproveAttributeUpgrade? improveAttributeUpdateChoice = ExtractUpgradeChoice<ImproveAttributeUpgrade>(update.Upgrade.Choice);
                    
                    if (improveAttributeUpdateChoice is null)
                    {
                        throw new BadHttpRequestException("Improve Attribute upgrade payload was incorrect. Please verify and try again.");
                    }
                    
                    int maxPropertyValue = improveAttributeUpdateChoice.AttributeOption switch
                    {
                ...
                finalizedChoice = ExtractUpgradeChoice<ImproveAttributeUpgrade>(update.Upgrade.Choice);
                break;
            case UpgradeOption.bonusFeature:
                // can't verify what's not there
                if (update.Upgrade.Choice is not null)
                {
                    BonusFeatureUpgrade? bonusFeatureUpdateChoice = ExtractUpgradeChoice<BonusFeatureUpgrade>(update.Upgrade.Choice);

                    if (bonusFeatureUpdateChoice is null)
                    {
                ...
                finalizedChoice = ExtractUpgradeChoice<BonusFeatureUpgrade>(update.Upgrade.Choice);
                break;
            case UpgradeOption.talent:
                if (update.Upgrade.Choice is not null)
                {
                    GainTalentUpgrade? talentUpdateChoice = ExtractUpgradeChoice<GainTalentUpgrade>(update.Upgrade.Choice);

                    if (talentUpdateChoice is null)
                    {
                        throw new BadHttpRequestException("Talent upgrade payload was incorrect. Please verify and try again.");
                    }
                    
                    if (talents.FirstOrDefault(x => x.Id == talentUpdateChoice.TalentId) is null)
                    {
                        throw new ValidationException([new ValidationFailure("Talent", $"Talent '{talentUpdateChoice.TalentId}' is invalid.")]);
                    }

                    if (character.Talents.FirstOrDefault(x => x.Id == talentUpdateChoice.TalentId) is not null)
                    {
                        throw new ValidationException([new ValidationFailure("TalentExists", "Talent already present on character.")]);
                    }
                }
                
                finalizedChoice = ExtractUpgradeChoice<GainTalentUpgrade>(update.Upgrade.Choice);
                break;
            case UpgradeOption.magicalPower:
                if (update.Upgrade.Choice is not null)
                {
                    NewMagicalPowerUpgrade? magicalPowerUpdateChoice = ExtractUpgradeChoice<NewMagicalPowerUpgrade>(update.Upgrade.Choice);
                    if (magicalPowerUpdateChoice is null)
                    {
                        throw new BadHttpRequestException("MagicalPower update payload was incorrect. Please verify and try again.");
                    }
                    
                    if (magicalPowers.FirstOrDefault(x => x.Id == magicalPowerUpdateChoice.MagicalPowerId) is null)
                    {
                        throw new ValidationException([new ValidationFailure("MagicalPower", $"Magical Power '{magicalPowerUpdateChoice.MagicalPowerId}' does not exist.")]);
                    }

                    if (character.MagicalPowers.FirstOrDefault(x => x.Id == magicalPowerUpdateChoice.MagicalPowerId) is not null
                        || character.MagicalPowers.FirstOrDefault(x => x.BonusFeatures.Any(y => y.Id == magicalPowerUpdateChoice.MagicalPowerId)) is not null)
                    {
                ...
                if (existingUpgrade is not null)
                {
                    ...
                    {
                        NewMagicalPowerUpgrade? existingMagicalPowerChoice = ExtractUpgradeChoice<NewMagicalPowerUpgrade>(existingUpgrade.Choice);
                        if (existingMagicalPowerChoice is null)
                        {
                            _logger.LogWarning("Existing Magical Power Choice update on character '{CharacterId}' was invalid and needs to be fixed", character.Id);
                        }
                        ...
                        foreach (Upgrade affectedUpgrade in affectedUpgrades)
                        {
                            ...
                            BonusFeatureUpgrade? upgradePayload = ExtractUpgradeChoice<BonusFeatureUpgrade>(affectedUpgrade.Choice);
                            if (upgradePayload!.MagicalPowerId == existingMagicalPowerChoice!.MagicalPowerId)
                            {
                ...
                finalizedChoice = ExtractUpgradeChoice<NewMagicalPowerUpgrade>(update.Upgrade.Choice);
                break;
            case UpgradeOption.owieLimit:
            case UpgradeOption.treatsValue:
                // these don't have validations
                break;
            default:
    ...
    private T? ExtractUpgradeChoice<T>(object? choice)
    {
        if (choice is null)
        {
            return default;
        ...
        try
        {
            return JsonSerializer.Deserialize<T>(choice.ToString()!, JsonSerializerOptions.Web);
        }
        catch
        {
            return default;
