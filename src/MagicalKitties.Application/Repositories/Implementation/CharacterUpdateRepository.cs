...
public class CharacterUpdateRepository : ICharacterUpdateRepository
{
    ...
    public async Task<bool> CopyCharacterInformation(Character existingCharacter, Character characterCopy, CancellationToken token = default)
    {
        using IDbConnection connection = await _dbConnectionFactory.CreateConnectionAsync(token);
        using IDbTransaction transaction = connection.BeginTransaction();
        ...
        int result = await connection.ExecuteAsyncWithRetry(new CommandDefinition("""
                                                                                  update characterstat
                                                                                  set level = @Level, current_xp = @CurrentXp, max_owies = @MaxOwies, current_owies = @CurrentOwies, starting_treats = @StartingTreats, current_injuries = @CurrentInjuries, cute = @Cute, cunning = @Cunning, fierce = @Fierce, incapacitated = @Incapacitated
                                                                                  where character_id = @CopyCharacterId
                                                                                  """, new
                                                                                       {
        ...
        if (result > 0)
        {
            if (existingCharacter.Talents.Count > 0)
            {
                    ...
                    if (result > 0)
                    {
                        ...
                        result = await connection.ExecuteAsyncWithRetry(new CommandDefinition("""
                                                                                          ...
                                                                                          """, new {
                                                                                                       Id = Guid.NewGuid(),
                                                                                                       CharacterId = characterCopy.Id,
                                                                                                       FlawId = existingCharacterTalent.Id
                                                                                                   }, cancellationToken: token));
                    }
                }
            }
        ...
        if (result > 0)
        {
            if (existingCharacter.MagicalPowers.Count > 0)
            {
                foreach (MagicalPower existingCharacterMagicalPower in existingCharacter.MagicalPowers)
                {
                    ...
                    {
                        result = await connection.ExecuteAsyncWithRetry(new CommandDefinition("""
                                                                                              insert into charactermagicalpower (id, character_id, magical_power_id, selected_bonuses)
                                                                                              values (@Id, @CharacterId, @MagicalPowerId, @SelectedBonuses)
                                                                                              """, new
                                                                                                   {
                                                                                                       Id = Guid.NewGuid(),
                                                                                                       CharacterId = characterCopy.Id,
                                                                                                       MagicalPowerId = existingCharacterMagicalPower.Id,
                                                                                                       SelectedBonuses = existingCharacterMagicalPower.BonusFeatures
                                                                                                   }, cancellationToken: token));
                    }
                }
            }
        ...
        // update stats, flaw, talent, magical powers, human, upgrades.

        if (result > 0)
        {
            if (existingCharacter.Humans.Count > 0)
            {
